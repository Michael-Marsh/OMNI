...
namespace OMNI.ViewModels
{
    public class RegistrationWindowViewModel : ViewModelBase
    {
        ...
        {
            if (OMNIDataBase.ExistsAsync("users", "EmployeeNumber", UserId.ToString()).Result)
            {
            ...
            {
                using (MySqlCommand cmd = new MySqlCommand("INSERT INTO omni.users (DomainName, FullName, AccountName, EmployeeNumber, eMail, Site) Values (@p1, @p2, @p3, @p4, @p5, @p6)", App.ConAsync))
                {
